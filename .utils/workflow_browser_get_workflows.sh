...
curl --location-trusted -H "X-Requested-Auth: Digest" -H "X-Opencast-Matterhorn-Authorization: true" --digest --user "${user}:${access}" "${server}/workflow/instances.json?state=&q=&fromdate=${datepast}&todate=${datenow}&sort=&startPage=0&count=800" > ${wfdump}${env}.json
